## 13 Sep 2013:
## a. Count number of new pregnancies here.
## b. Implement immaculate conception in "dc". 
...
assign.pregnancy <-
  ...
  ## Now think about how to populate prob-vector.
    ...
    preg.crit.true <- intersect3(c(lt.preg.thres,
                                  never.preg.fem),
                                female.in.rel,
                                 curr.not.pregnant
                                )
   ...
   ######################################################################## 
    ...
    for (i in 1:length(female.in.rel)){
      ...
      if (female.in.rel[i] %in% preg.crit.true) {
        curr.pregnancy.status[female.in.rel[i]] <-
          rbinom(1, 1, preg.prob[female.in.rel[i]])

        if (!is.na(curr.pregnancy.status[female.in.rel[i]])){
                                        #3Sep13: to protect comparison
                                        # with NA's
          if (curr.pregnancy.status[female.in.rel[i]] == 1){
            time.since.curr.pregnancy[female.in.rel[i]] <- 0
            time.of.curr.pregnancy[female.in.rel[i]] <- time
            time.since.last.pregnancy[female.in.rel[i]] <- NA
        ...
        ## it can keep incrementing through the loop?
          ...
          if (verbose==TRUE && curr.pregnancy.status[female.in.rel[i]]
              == 1){
