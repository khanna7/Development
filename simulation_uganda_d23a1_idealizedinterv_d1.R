...
#####################################################
  ...
  rm(list=ls())
      ...
      # load("estimation_uganda_d9a_agemix_diagstrzero_restptdurn.RData") #4Oct13
        ...
        load("nw13Oct13_rep_mem_error_trialrun1.RData") #13Oct13
...
#####################################################
    ...
    date <- "13Oct13_rep_mem_error_trialrun1"
## date <- "12Oct13_rep_mem_error_trialrun4"
...
#####################################################
  ...
  ## Fit, formula, population features
      ## nw <- fit$network
##         nw <- network.collapse(nw, at=1300) #13Oct13x
...
#   for (time in 1502:1560) {
#   for (time in 1301:1560) {
    for (time in 1555:1560) {
    ...
    nw <- compute.cd4.count.sexage.acct(nw, verbose=FALSE,
      ...
      ## Update edges coef, give feedback
        ...
        if (time >= 1380){ ## 13Oct13
          if (time %% 5 == 0){
            nw <- network.collapse(nw, at=time) ##11Oct13
            cleanMem()
          }
        } else if (time < 1380){
            if (time %% 10 == 0){#12Oct13 -- try collapsing every 10th timestep
              nw <- network.collapse(nw, at=time) ##11Oct13
              cleanMem()
        }
          }
...
#####################################################
 ...
 ## of here to reduce its total size.
  ...
  ## source("common.functions_d8.R")
  ## cleanMem()
  ## load("id_interv_simulation.10Oct13_rep_mem_error_trialrun1.RData")
  ## nw <- network.collapse(nw, at=1547)
  #nw <- network.collapse(nw, onset=1400, terminus=Inf)
