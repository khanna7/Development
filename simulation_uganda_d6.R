## 4 June 2013: Try test-based development,
## removing the networks from being inserted as
## an argument inside of main functions
## also update the estimation file to "estimation_uganda_d4.RData"
...
rm(list=ls())
     ...
     ## SIMULATION-related files and packages
       ...
       source("update.vital.dynamics_d4.R")
       source("update.biological.parameters_d4.R")
...
## debug(transmission)
    ...
    for (time in 2:timesteps) {   
      ...
      ## 4 June 2013: to write functions that are easier to test
      ## by avoiding passing the entire network through the functions
      ## that are below.
      ## first step in this process is to extract the xn network
      ## at the timestep in the loop that we are going through.
      
      nw.xn <- network.extract(nw,
                               at = time,
                               retain.all.vertices = T)

      ## extract all relevant attributes

      ## data to extract gender
      male.id.today <- nwmodes(nw.xn, 1)
      female.id.today <- nwmodes(nw.xn, 2)


      ## data to apply "update.biological.parameters,
      ## "compute.cd4" and "compute.viral.load" functions
      inf.status <- nw.xn%v%"inf.status"
      art.status <- nw.xn%v%"art.status"
      time.since.infection <- nw.xn%v%"time.since.infection" 
      time.since.art.initiation <- nw.xn%v%"time.since.art.initiation" 
      cd4.count.today <- nw.xn%v%"cd4.count.today"
      viral.load.today <- nw.xn%v%"viral.load.today"
      male.cd4.today <- cd4.count.today[male.id.today]
      female.cd4.today <- cd4.count.today[female.id.today]
      pregnant.today <- which(nw%v%"pregnant" == 1)
     ...
     ## UPDATE VITAL DYNAMICS, AND UPDATE NETWORK
      nw <-
        update.biological.parameters.tdd(male.id.today=male.id.today, 
                                     female.id.today=female.id.today, 
                                     inf.status=inf.status,
                                     art.status=art.status,
                                     time.since.infection=time.since.infection,
                                     time.since.art.initiation=
                                     time.since.art.initiation,
                                     cd4.count.today=cd4.count.today,
                                     viral.load.today=viral.load.today,
                                     male.cd4.today=male.cd4.today,
                                     female.cd4.today=female.cd4.today,
                                     pregnant.today=pregnant.today,
                                     verbose=TRUE,
                                     scenario="baseline",
                                     dur.inf=dur.inf,
