...
## assigned.
    ...
    ##number of days since last breastfeeding
      ## min.preg.interval.p1 <- (15*30/14)+1

      n0%v%"time.of.curr.pregnancy" <- NA
      n0%v%"time.since.curr.prenancy" <- NA
      n0%v%"time.since.last.prenancy" <- NA
      ...
      set.vertex.attribute(n0, "time.of.curr.pregnancy", 0,
                           pregnant.yes)
      ## set.vertex.attribute(n0, "time.since.last.pregnancy",
      ##                      min.preg.interval.p1,
      ##                      female.id)
...
##############
  ...
  ## for chronically infected
     ...
     ## change lengths due to size of timestep
 ...
 ## Time since ART initiation
    ### Initially, for all people on ART set=0?
        ...
        time.of.art.initiation <- time.since.art.initiation
                                        # both set to 0 at start

        art.type <- rep(NA, N)
        art.type[on.art] <- 1 #everyone at time 0 has regular ART
       ...
       if (init.inf.status[i] == 1) { # untreated individuals
         cd4.count.today[i] <-
           cd4.at.infection.male - # this is a constant value
             ...
             time.since.infection[i]*untreated.cd4.perstep.decline
                                    # change due to size of timestep
       } 
  ...
  ## Viral Load Today
     ## Relevant Parameters
     ## change due to different size of timestep
     ...
     time.infection.to.peak.viral.load <- 14/size.of.timestep
     peak.viral.load <- 6.17
     time.infection.to.viral.set.point <- 121/size.of.timestep
     set.point.viral.load <- 4.2
     time.infection.to.late.stage <- 1877/size.of.timestep
    ...
    ## 12Jun13: Assign viral load change slopes
...
## Save Object
#####################################################
    save.image(file="estimation_uganda_d4.RData")
#####################################################
