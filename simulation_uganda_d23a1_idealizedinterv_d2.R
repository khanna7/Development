...
## 9 Oct 2013: Try breaking down the intervention simulation

## 8 Oct 2013: Memory problems are still showing up -- one thing to do might be
## to try collapsing the network at every time step. 

## 7 Oct 2013:
## a. Implement baseline intervention starting with network
## object from burnin run, in this case --
## "simulation.4Oct13_inc_fert_rate_wagemix_diagstrzero_age15to55_1300steps_run1.RData"

## b. Collapse the starting network to timestep 1300.
## nw <- network.collapse(nw, at=1300)

## c. Then implement the idealized conditions:
## CD4 eligibility: 350 -- does not change
## ART initiation: 3 months after eligility
## ART coverage: 80%
## PMTCT eligibility: WHO Option A (does not change)
## PMTCT initiation: 14 weeks
## PMTCT coverage: 90%

## d. NEED TO ADD attributes "time.of.art.eligibility" and "time.since.art.eligibility"
## to the estimation code, and use it in burnin. For now, as a measure of CD4 count
## 3 months after eligibility, use CD4 = 350 - (90*0.14) = 338.

## e. Running into memory problems. Try deleting "nw.start" and "nw.stats" because they
## are the next biggest memory hogs, besides "nw" itself. 
...
#####################################################
  ...
  rm(list=ls())
      ...
      # load(file="estimation_uganda_d9_agemix_nodiff.RData") #30sep13
      # load("estimation_uganda_d9a_agemix_diagstrzero.RData")
      # load("estimation_uganda_d9a_agemix_diagstrzero_restptdurn.RData") #4Oct13

        load("simulation.4Oct13_inc_fert_rate_wagemix_diagstrzero_age15to55_1300steps_run1.RData")
  ...
  ## SIMULATION-Related Files and Packages
      ...
      source("common.functions_d8.R")
       ...
       # source("update.vital.dynamics_d10a.R") # 29Aug13: 0 and 1 to repr. males and feml.
         source("update.vital.dynamics_d11a.R") # 30Sep13:with "age.cat" variable
       # source("update.vital.dynamics_d11a_nonetextract.R.R")
                                       # 10Oct13
       ...
       # source("assign.pregnancy_d5d1_nonetextract.R") #9Oct13
       ...
       # source("update.treatment_d6_nonetextract.R") # 9Oct13
       ...
       # source("transmission_d9_nonetextract.R")
...
## date <- "10Oct13_idealizedinterv_diagstrzero_age15to55_collapsenet_del_lgobj_seqofsteps_260steps_run2"
  date <- "10Oct13_trial_to_rep_error_fromfunccall_parallelize_trial1"
...
#####################################################
  ...
  scenario <- "idealized_intervention"
  # baseline.preg.coverage.rate <- baseline.preg.art.coverage.rate
  baseline.preg.coverage.rate <- 0.90 #7Oct13
...
#######################################################
   ...
   seq.of.steps <- function(net, time1, time2){
   ...
   for (time in time1:time2) {   
    ...
    nw <- compute.cd4.count.sexage.acct(nw, verbose=FALSE,
      ...
      ## Update edges coef, give feedback
        ...
        if (time == time1){ #7Oct2013: for memory management
     ...
     # nw <- network.extract(nw, at=time, retain.all.vertices=TRUE) #10Oct13
    }
   return(nw)
 }

  ## using network.extract this way causes the number of edges to fall drastically
  nw <- seq.of.steps(net=nw, time1=1301, time2=1350)
  nw <- network.collapse(nw, at=1350)

  ## nw <- seq.of.steps(net=nw, time1=1304, time2=1305)
  ## nw <- network.extract(nw, at=1305)
  ## nw <- seq.of.steps(net=nw, time1=1306, time2=1350)
  ## nw <- network.extract(nw, at=1350)
  ## nw <- seq.of.steps(net=nw, time1=1351, time2=1400)

  ## nw <- seq.of.steps(net=nw, time1=1301, time2=1560)
  #rm(list=ls())

  ## load("nw10Oct13_idealizedinterv_diagstrzero_age15to55_collapsenet_del_lgobj_seqofsteps_260steps_run2.RData")

  nw <- network.collapse(nw, at=1398)

  nw <- seq.of.steps(time1=1399, time2=1410)

  save.image(paste(file="id_interv_simulation.", date, ".RData", sep="")) #5 Aug 2013
