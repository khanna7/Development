...
library(ergm)#31 Aug 2013: In 64-bit I don't have statnet-3.1, but I have ergm-3.1
library(tergm)
...
## 31 Aug 2013: Compile on 64-bit Libra.
...
#####################################################
      ...
      male.id <- which(n0%v%"sex"==0)
      female.id <- which(n0%v%"sex"==1)
  ...
  ## BIOLOGICAL ATTRIBUTES 
      ...
      ## Infection Status 
      init.inf.status <- rbinom(network.size(n0), 1, init.hiv.prev.15)
...
## Save Object
#####################################################
     save.image(file="estimation_uganda_d8d_rmvstringatt_64bit.RData")
#####################################################
