...
## 12 March 2014: Limit burnin to 30 years

## 23 Feb 2014: Increase duration by 1000 days.

## 21 Feb 2014:
## a. With new attributes
## b. Add "sc.art.postcess.ret.bl" to 'compute.cd4.count"
## function
## c. Put in new estimation object
## d. Add both arguments "optB.cess.time" and "optB.vl.postcess.ret.bl"
## to "assign.pregnancy" and the latter and
## "sc.art.postcess.ret.bl"
## to "compute.cd4.count" functions
## e. date-stamp on data is correct -- 21 Feb, 
##    i did couple other runs for 100 time steps with date
##    "22Feb"

## 12 Feb 2014: Apply return to pre-ART viral load
## in 1 month for women who have ceased ART

## 10 Feb 2014: Increase duration by 700

## 6 Feb 2014: Attempt to reduce incidence in ZA

## 30 Jan 2014:
## a. Fix sign  of "vl.art.traj.slope"
## version: "update.treatment_d10.R"
## b. Also had to change initial network object from
## estimation because assignment of initial "vl.art.traj.slope"
## in the initial network object was wrong.

## 16 Jan 2014:
## a. Adapt Fielding estimates for mortality
## for South Africa.

## b. Reduce burnin to 1040 timesteps, i.e. 40 years.
## will save time at the end of the intervention simulations.

## 15 Jan 2014: Rerun model with Fielding estimates for pre-ART mortality

## 14 Jan 2014: Rerun burnins with mortality for
## positives rewritten:
## a. Partitioned mortality of positive, ART-naive
## individuals by CD4 counts ("common.functions_d9.R").
## b. Mortality of all infectives adjusted by size of timestep...
## source("../update.vital.dynamics_newinfected_entryage18_d13a.R")#2Dec13

## 14 Dec 2013: Run burnin to 1100 timesteps --
## burnins that go upto 1700 timesteps are too slow later.
...
## STATNET-Related Files and Packages
 ...
 ## Starting Network
      load("estimation_za_d9d_vl.art.traj.slope_dur1000_newatts.RData") #3Mar14
  ...
  ## SIMULATION-Related Files and Packages
       ...
       source("../params_za_d2.R") # ultimately take this out of here -- should
...
#####################################################
  ...
  date <- "12March_ZA_phi004_incdur1000_newatts_30yrburn_run3"
...
#####################################################
  ...
  ## Other Needed Parameters for timesteps
      ## timesteps <- 1040 ## 21Feb14w
      timesteps <- 26*30 ## 12Mar14
      start.time <- 1
...
##debug(update.vital.dynamics)
    ...
    for (time in 2:timesteps) {
      ...
      nw <- assign.pregnancy(nw, verbose=TRUE,
                             ...
                             sc.art.postcess.ret.bl=
                             sc.art.postcess.ret.bl.chung,
                             num.births.per1k.byage=
                             num.births.per1k.byage.15pcinc,
                             prop.stillbirth=prop.stillbirth, 
                             inf.preg.red=inf.preg.red,
                             scenario=scenario,
                             optB.cess.time=optB.cess.time, #21Feb14
                             optB.vl.postcess.ret.bl=optB.vl.postcess.ret.bl #21Feb14
                             )
        ...
        nw <- compute.cd4.count.sexage.acct(nw, verbose=FALSE,
                                               ...
                                               sc.art.postcess.ret.bl=
                                               sc.art.postcess.ret.bl.chung, #21Feb14
                                               optB.vl.postcess.ret.bl=optB.vl.postcess.ret.bl#21Feb14
                                               )
        ...
        nw <- compute.vl.mp.art4(nw, verbose=FALSE, 
                                 ...
                                 undetectable.vl,
                                 sc.art.postcess.ret.bl=
                                 sc.art.postcess.ret.bl.chung, #25Jul13
                                 size.of.timestep=size.of.timestep, # 28May13
                                 )
  ...
  save.image(paste(file="burnin.", date, ".RData", sep="")) #5 Aug 2013
