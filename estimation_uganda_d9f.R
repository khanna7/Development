...
#source("params_d1.R")
source("params_d2.R") #22Aug13 age based duration of infection
...
## 1 Oct 2013:
## a. Adding nodematch-type age mixing for bipartite networks
...
## b. Specifying diagonal entries, and structural zeros works. But we get
## fewer entries than specified between M3-W1, and more than specified in
## M4-W1. Try to fix this by specifying these two entries as well.
## SPECIFYING THESE TWO ADDITIONAL ENTRIES PRODUCES MEMORY PROBLEMS:
## ``Error: cannot allocate vector of size 667.6 Mb''

## Version (c): no age mixing

## Version (d): lower right triangle of matrix, in addition to
## diagonal (instead of structural zeros in upper-left corner)

## Version (e): lower right triangle of matrix, upper left triangles
## (no diagonals)

## Version (f): With version (e), add duration from partnerships
## restricted to 15 to 55.
...
#####################################################
  ...
  seq.of.age.mix <- 1:16
  # to.specify <- c(1, 6, 9, 11, 13, 14, 16) # To specify: str zeros --
                                           # which(age.mix.all.stat.trans == 0) below,
                                           # and,
                                           # diagonal entries
  to.specify <- c(3, 4, 7, 9, 13, 14)
  ...
  age.mixing.all.stat.trans <- t(age.mixing.all.stat)
  age.mixing.all.stat.trans <- c(age.mixing.all.stat.trans)
  age.mix.spec.diag.strzero <- c(393, 453, 0, 201, 0, 0, 73)
  age.mix.spec.diag.strzero.mw31.41 <- c(393, 35, 37, 453, 0, 201, 0, 0, 73)

  age.mixing.all.stat.trans.str.zero <- c(age.mixing.all.stat.trans)
  age.mix.spec.diag.lowtriangle <- c(393, 35, 37, 453, 169, 201, 73)
  age.mix.spec.uptriangle.lowtriangle <- c(35, 37, 169, 0, 0, 0)
  ...
  age.mixing.b1fact <- c(426,
                         837,
                         480,
                         254)

  target.stats.with.age.mixing <- c(target.stats,
                                    #age.mixing.all.stat[-1]) #30Sep13
                                    age.mixing.all.stat.trans[-1]) #1Oct13
                                    
  target.stats.with.age.mix.nodiff <- c(target.stats,
                                        age.mixing.nodiff[-1])

  target.stats.with.age.mix.nodiff.b1fac <- c(target.stats,
                                              age.mixing.nodiff[-1],
                                              age.mixing.b1fact[-1])

 target.stats.with.age.diag.strzero <- c(target.stats,
                                          age.mix.spec.diag.strzero)

 target.stats.age.mix.spec.diag.strzero.mw31.41 <- c(target.stats,
                                                     age.mix.spec.diag.strzero.mw31.41)

 target.stats.with.age.diag.lowtriangle <- c(target.stats,
                                             age.mix.spec.diag.lowtriangle)

 target.stats.with.age.mix.spec.uptriangle.lowtriangle <- c(target.stats,
                                                            age.mix.spec.uptriangle.lowtriangle)
...
#####################################################
  ...
  fit <- ergm(formation.n0,
              target.stats=
              #target.stats,
              #target.stats.with.age.mixing, #30Sep2013
              #target.stats.with.age.mix.nodiff, #30Sep2013
              #target.stats.with.age.mix.nodiff.b1fac, #30Sep2013
              #target.stats.with.age.diag.strzero,#1Oct13
              #target.stats.age.mix.spec.diag.strzero.mw31.41,#1Oct13
              #target.stats.with.age.diag.lowtriangle,
              target.stats.with.age.mix.spec.uptriangle.lowtriangle,
  ...
  hetdeg.diag.sim.est9e <- simulate(n0,
   ...
   save(hetdeg.diag.sim.est9e, file="hetdeg.diag.sim.est9e.RData")
...
## Save Object
#####################################################
  save.image(file="estimation_uganda_d9f_agemix_uptriangle_lowtriangle_durn15to55.RData")
#####################################################



## 1Oct2013: test age mixing
age.mix.mat <- mixingmatrix(hetdeg.diag.sim.est9e, "age.cat")$matrix
