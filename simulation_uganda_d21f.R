...
## 30 Aug 2013:
## a. prespecify nw below, so R does not have to find a large memory
## chunk for it later. -- did not work since nw is not a matrix.
## b. try runnig gc() periodically -- not sure this will work either.
...
date <- "30Aug13_rungcrep_64bitonlibra_15pc_10phi_1300steps_run1"
         # check if epidemic dies after 1000 timesteps

#####################################################


##nw <- matrix(NA, nrow=1.5e4, ncol=1.5e4) #30Aug13: prespecify "nw"
# so R does not need to find a memory chunk late
  ...
  for (time in 2:timesteps) {   
    ...
    nw <- compute.cd4.count.sexage.acct(nw, verbose=FALSE,
      ...
      ## Update edges coef, give feedback
        ...
        print(as.matrix(rev(sort(z))[1:10]), units="auto")
