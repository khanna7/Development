## 29 Sep 2013: More corrections are needed to restrict the network only
## to cross-section.

## 25 Sep 2013: The correction factor should only take ratios of populations
## in the cross-section at a given time. I was taking the
## counts of females in the entire network.
## I had tried this earlier -- 16 Sep 2013 --
## but I called the cross-sectional network "nw" -- which may have been a mistake
## because then I lose the information in the complete network.

## 22 Sep 2013: Try correcting pregnancy probability on "assign.pregnancy_d5d1.R" --
## starting "assign.pregnancy_d5d2.R"
...
assign.pregnancy <-
  function(nw, verbose=TRUE,
    ...
    cross.net <- network.extract(nw, at=time) #25Sep13
    ## male.id.curr <- nwmodes(nw, 1)
    ## female.id.curr <- nwmodes(nw, 2)
    male.id.curr <- nwmodes(cross.net, 1) #25Sep13
    female.id.curr <- nwmodes(cross.net, 2) #25Sep13

    curr.pregnancy.status <- cross.net%v%"curr.pregnancy.status"
    time.since.curr.pregnancy <- cross.net%v%"time.since.curr.pregnancy"
    time.of.curr.pregnancy <- cross.net%v%"time.of.curr.pregnancy"
    time.since.last.pregnancy <- cross.net%v%"time.since.last.pregnancy"
    art.status <- cross.net%v%"art.status"
    art.type <- cross.net%v%"art.type"
    vl.art.traj.slope <- cross.net%v%"vl.art.traj.slope"

    time.of.art.cessation <- cross.net%v%"time.of.art.cessation" #25Jul13
    time.since.art.cessation <- cross.net%v%"time.since.art.cessation" #25Jul13
  ...
  ## Now think about how to populate prob-vector.
    ...
    cat("Number of women matching preg criteria in each age category ",
      ...
      prob.year = (num.births.per1k.byage/(1-prop.stillbirth))*F*1/f
                                        # 22Sep2013: bring back correction factor
