...
## 23 May: Reduced number of isolates and increased number of women with deg 1
## by 20% 
## to match total degrees for men. 
## Constrain max degrees?

## THE STANDARD DEVIATION OF PARTNERSHIP DURATIONS APPEARS TO BE TOO HIGH.
## Remember it is better to look at a subset of the partnership durations in the
## simulated ergm data, since a lot of the later modeled partnerships began late,
## they will not be anywhere near completion. 

## 21 May 2013: Adapt for estimation of Ugandan network
...
#####################################################
  ...
  #### Network Related Statistics
        ...
        ## male.deg.dist <- c(0.48,0.41,0.08,0.03) ## hypothetical working values
        ## female.deg.dist <- c(0.40, 0.55, 0.04, 0.01)

        ## duration <- 4323 ## Community Survey Data
        duration <- 100 ## Community Survey Data
	diagnostics <- T
	...
	male.deg.tot <- (0*male.deg.counts[1] + 1*male.deg.counts[2] +
      ...
      ## other behavioral information (from data)
         ## number of sex partners
         ## male (n=948)
         male.tot.deg.emp <- (0*330)+(1*496)+(2*93)+(3*18)+(4*7)+(5*7)+(8*1)
                                        # total degree
         male.mean.deg.emp <- male.tot.deg.emp/(948-2)
                                        # ignoring don't knows in denominator

         ## male (n=1148)
         fem.tot.deg.emp <- (0*495)+(1*657)+(2*12)+(3*1)+(7*1)+(8*1)+(20*1)
         fem.mean.deg.emp <- fem.tot.deg.emp/(1148-10)     


         ## male.deg.tot.emp <- c((0.34*2500*0), (0.52*2500*1))
...
### Set up model
	...
	## target.stats <- c(female.deg.tot, female.deg.counts[1:2], male.deg.counts[1:2])
        target.stats <- c(male.deg.tot, male.deg.counts[1:2],
...
## assigned.
    ...
    fem.pregnancy.na <- which( which(is.na(n0%v%"pregnancy.status")) > N/2)
    get.vertex.attribute(n0, "age")[fem.pregnancy.na]
...
##############
  ...
  ## Time Since Infection 
     ...
     time.since.infection.val <- sample(duration.of.infection,
     ...
     ## time.since.infection[which(init.inf.status == 1)] <- 0
     time.since.infection[which(init.inf.status == 1)] <- time.since.infection.val
     ## in proportion to duration of infection
  ...
  ## for chronically infected
     ...
     stage.of.infection <- rep(NA, N)
     ## stage.of.infection <- unlist(lapply(time.since.infection, classify.stage.numeric))
  ...
  ## Diagnosis Status,
     ...
     ## testing probabilities are not needed
     ...
     ## INSTEAD OF MODELING DIAGNOSIS STATUS, WE CAN JUST MODEL WHETHER SOMEONE 
     ## IS ON TREATMENT OR NOT 
 ...
 ## ART Status (Whether currently on treatment
   ...
   ##art.status[art.status<3] <- 0 ## 7 April 2013, see above
    ...
    art.status <- rep(NA, N)
    art.status[which(init.inf.status==1)] <- 0 ## no one is initially on ART
 ...
 ## Time since ART initiation
  ...
  ## WE ARE NOT MODELING HSV STATUS AND DYNAMICS EXPLICITLY
  ...
  ## Viral Load Today
     ...
     ## viral.load.today
      ...
      viral.load.today <- compute.viral.load(n0, verbose=FALSE,
      ...
      acute.mult <- 4.98
      late.mult <- 3.49
      infectivity.today <- assign.infectivity(n0, verbose=TRUE,
                                              min.chronic.infectivity=
                                              min.chronic.infectivity,
                                              acute.mult=acute.mult,
                                              late.mult=late.mult)

      n0%v%"infectivity.today" <- infectivity.today
...
#####################################################
   ...
   ## Save simulation object -- takes a long time to run (~20 mins with
     ...
     save(hetdeg.diag.sim, file="hetdeg.diag.sim.uganda.RData")
...
## Save Object
#####################################################
    save.image(file="estimation_uganda_d1.RData")
#####################################################
