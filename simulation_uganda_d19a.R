...
#####################################################
  ...
  rm(list=ls())
      # load("estimation_uganda_d6.RData") 
        load("estimation_uganda_d6a.RData") #20Aug13:Simulate with 10% beginning infection
  ...
  ## SIMULATION-Related Files and Packages
      #source("common.functions_d7.R")
       ...
       source("update.vital.dynamics_d8.R") 
       ...
       source("transmission_d8.R")

  ## Starting Network 
      # load("estimation_uganda_d5.RData")
      
       source("params_d1.R") # ultimately take this out of here -- should
...
date <- "20Aug13_sep_v31_fix_recrtmt_3phi_10pc_run2"
...
#####################################################
  ...
  ## Other Needed Parameters for timesteps
      timesteps <- 1000 ## 27 May '13: this mean 100*14 days now
      start.time <- 1
