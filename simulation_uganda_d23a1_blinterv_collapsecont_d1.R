...
## b. Collapsing the network seems to work, and reduces simulation time greatly.

## c. Running into memory problems. Try deleting "nw.start" and "nw.stats" because they
...
date <- "8Oct13_blinterv_diagstrzero_age15to55_collapsenet_contn_del_lgobj_260steps_run2"
...
#####################################################
  ...
  scenario <- "baseline_intervention"
...
#######################################################
  ...
  for (time in 1301:timesteps) {   
    ...
    nw <- compute.cd4.count.sexage.acct(nw, verbose=FALSE,
      ...
      ## Update edges coef, give feedback
        ...
        if (time  == 1305){ #7Oct2013: for memory management
  ...
  save.image(paste(file="bl_interv_simulation.", date, ".RData", sep="")) #5 Aug 2013
