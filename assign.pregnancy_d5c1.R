  ...
  ## Now think about how to populate prob-vector.
   ...
   ######################################################################## 
    ...
    for (i in 1:length(female.id.curr)){
      ...
      ## if (curr.pregnancy.status[female.id.curr[i]] == 0 &&
      ##     (is.na(time.since.last.pregnancy[female.id.curr[i]]) || 
      ##      time.since.last.pregnancy[female.id.curr[i]] > min.preg.interval)
      ##     ) {
