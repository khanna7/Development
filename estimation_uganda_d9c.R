...
#####################################################
  ...
  formation.wo.age.mix <- ~edges+b1degree(0:1)+b2degree(0:1)
              

  #formation.n0 <- update.formula(formation,n0~.)
  formation.n0 <- update.formula(formation.wo.age.mix, n0~.)
...
#####################################################
  ...
  hetdeg.diag.sim.est9c <- simulate(n0,
                              formation=formation.wo.age.mix,
   ...
   save(hetdeg.diag.sim.est9c, file="hetdeg.diag.sim.est9b.RData")
...
## Save Object
#####################################################
  save.image(file="estimation_uganda_d9c_noagemix.RData")
#####################################################

## 1Oct2013: test age mixing
age.mix.mat <- mixingmatrix(hetdeg.diag.sim.est9c, "age.cat")$matrix
