## 29 Sep 2013: Try debugging here to see why simulated number of pregnancies
## with correction is lower than simulated number of pregnancies without
## correction.
## a. Change name of variable from "F" to "F.list" and "f" to "f.list."
## b. More importantly, take ratios of LENGTHS of F.list and f.list --
## i.e. length(F)*1/length(f), instead of saying F*1/f
## THIS SHOULD NOT ONLY BE LENGTHS -- the vectors F and f both have length
## 7 -- because there are 7 age categories where pregnancy is possible. 
...
assign.pregnancy <-
  ...
  ## Now think about how to populate prob-vector.
    ...
    F.list <- c(F1, F2, F3, F4, F5, F6, F7) #29Sep13: change varname to avoid
    f.list <- c(f1, f2, f3, f4, f5, f6, f7) # confusion with "FALSE"
    ...
    cat("Number of women matching preg criteria in each age category ",
        ...
        ## Now probi = (w_i/1-v)*Fi*1/fi where 1 <= i <= 7.
         ...
         prob.year = (num.births.per1k.byage/(1-prop.stillbirth))*
           F.list*1/f.list
      ## 29Sep13: a. Change name of variable from "F" to "F.list"
      ##          b. Take ratios of lengths of F.list and f.list
    ...
    cat("Annual pregnancy rates per 1000 are ", prob.year, "\n")
        ## F and f are vectors
