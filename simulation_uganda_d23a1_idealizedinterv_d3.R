...
## 11 Oct 2013: Breakdown and automate the process of releasing memory
## to automate process of memory management
...
date <- "11Oct13_breakdown_automate_trialrun1"
...
#  for (time in 1301:timesteps) {

seq.of.steps <- function(nw=nw, time1, time2, date){
  ...
  for (time in time1:time2) {
    ...
    nw <- compute.cd4.count.sexage.acct(nw, verbose=FALSE,
      ...
      ## Update edges coef, give feedback
        ...
        if (time == 1301){ #7Oct2013: for memory management
      ...
      cat("\n")
        ...
        save.image(paste(file="id_interv_simulation.", date, ".RData", sep=""))
        save(nw, file=paste("netdyn_id_interv_simulation.", date, ".RData", sep=""))
    }
  ## save.image(paste(file="id_interv_simulation.", date, ".RData", sep=""))
  ## return(nw)
}
...
#####################################################
 ## 10 Oct 2013
 ## See if you can apply "network.collapse" to "nw" object that comes out
 ## of here to reduce its total size.
  seq.of.steps(nw, time1=1301, time2=1305, date="11Oct13_breakdown_automate_trialrun1")
  ...
  source("transmission_d9_nonetextract.R")
  date="11Oct13_breakdown_automate_trialrun1"
  load(paste(file="id_interv_simulation.", date, ".RData", sep=""))
  load(paste(file="netdyn_id_interv_simulation.", date, ".RData", sep=""))
  nw <- network.collapse(nw, at=1305)
  seq.of.steps(nw, time1=1306, time2=1350, date="11Oct13_breakdown_automate_trialrun1")
  ...
  seq.of.steps(nw, time1=1351, time2=1400, date="11Oct13_breakdown_automate_trialrun1")
