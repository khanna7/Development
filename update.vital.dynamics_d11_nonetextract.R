    ...
    ## 8 Oct 2013: Since we are collapsing the network at every time step now,
    ## comment out "network.extract"
    ...
    ##                           is.active(nw,v=1:network.size(nw), at=time))
     ...
     # 26 Aug 2013: Update active nodes
       ...
       ##  age.f.curr <- nw%v%"age"[which(get.vertex.attribute(nw,'sex')=="female")]
           #nw.curr.wo.dead.nodes <- network.extract(nw, at=time) #8Oct2013
           nw.curr.wo.dead.nodes <- nw #8Oct2013 -- try no network extract
      ...
      ## } # This loop approach is temp to get around bug in network
       ...
       if (length(nodes.to.activate)>0) { ## if new nodes enter the network
         ...
         ## update relevant attributes here 
          ...
          nw <- set.vertex.attribute(nw, "age.cat", 0, nodes.to.activate)
               ## 30Sep13: set initial age category to 0,
               ## since all new ages are 15 
          ...
          ########################################################
          ## 13 Aug 2013:
          ## Comment out section below and move to
          ## time of infection in transmission file.
          ########################################################
          
          ## if (scenario == "baseline"){

          ##  art.covered <- rbinom(length(nodes.to.activate), 1,
          ##                         baseline.art.coverage.rate)

          ##   preg.covered <- rbinom(length(new.female.nodes), 1,
          ##                          baseline.preg.coverage.rate)
