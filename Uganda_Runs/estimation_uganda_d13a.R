...
## 21 Feb 2014: Add new attributes,
## for differentiating viral load and CD4 count trajectories
## after Option A and Option B

## 17 Feb 2014: I was thinking I had to fix
## degree distribution in estimation code by
## changing "male.deg.counts[1:2]" in "target.stats" to
## "male.deg.tot[1:2]". But no. As currently specified,
## 34.8% of men get 0 partners, and 52.2% get
## 1 partner. Similarly, as specifed, 22% of women get 0
## partners, and 75% get 1 partner. So nothing needs to be
## changed in the estimation routine. 

# 30 Jan 2014: Clear bugs in "vl.art.traj.slope" --
## was assigned to NA for those initially infected

## 4 Dec 2013: Initialize "num.of.pregnancies" attribute to 0 for everyone,
## and change to 1 for those who are pregnant initially.
## Previous object -- "estimation_uganda_d10a_agemix_diag_b1fact_b2fact_minage18_18nov13.RData" -- contains exactly the same information, 
## without the number of pregnancies. 
## New estimation object is in "(file="estimation_uganda_d11a.RData")".

## It appears that ergm version has been updated to 3.1-1. I had been using 3.1-0 so
## far. Should revert to 3.1-0 as soon as possible.

## Reverted to version ergm version 3.1-0. This does not affect anything with the
## South Africa runs started on 3 Dec 2013 because I started them in the morning
## and the estimation code would not have compiled if 3.1-1 were being used. 
## And the problem (i.e. installation of 3.1-1 occured) because Matt updated the
## site library last night. 

## 18 Nov 2013:
## a. Fix age distribution to account for 18 as age at entry
## (done in "../params_uganda_d3.R".
## b. Change parameter to "preg.susc.mult" in transmission and "preg.mult" in assign
## infectivity (in burnin and simulate files)? 
## c. multiply fertility parameters by size of timestep

##  12 Nov 13:
## should fertility parameters below be adjusted for timestep -- i.e. multiply by 14?

## 11 Nov 2013:
## a.  Since working on the ZA model I had been concerned, because I
## felt that the ZA network model build at the start was more principled,
## and I had examined its structure more closely at the beginning. I was
## afraid that there was probably a long tail lurking in the degree
## distributions, pr some other artifact that we
## didn't want to see. So I looked at the two parameters of interest:
## the degree distributions,
## and the age mixing matrix.  I think both look quite good, and I don't
## need to rewrite how the initial UG model is structured at the
## beginning. I was especially worried about the degree distributions
## since I did not explicitly constrain the max degrees here as I did in
## the ZA model, and because I made the degree distributions work by
## reducing the number of isloates for women, and increasing the number
## of women with degree 1.

## See results from a network at the tenth time step simulated without any vital demographic or processes below (similar to results I was emailing you earlier about ZA):
...
##     To
## From           1           2           3           4
##    1 0.192460317 0.019345238 0.002976190 0.001984127
##    2 0.124007937 0.228174603 0.045634921 0.021329365
##    3 0.060019841 0.062996032 0.099702381 0.012896825
##    4 0.034722222 0.039186508 0.017361111 0.037202381 

## b. Clean up the age mixing code.

## c. After cleaning up the age-mixing code, model would not converge if only one
## rowsum (b1factor) and colsum (b2factor) was left out. Therefore, I left two out,
## and still got good looking simulated  simulated degree distributions and
## age-mixing.

## 7 Nov 2013: Change age at entry to 18.
## Use revised age mixing matrices from Sarah (email dated 6 Nov 2013).

## 3 Nov 2013: To investigate long term behavior of prevalence when
## only diagonal entries in the age-mixing matrix are specified.

## 1 Nov 2013:
## a. Recode values of attribute sex to "male" and "female"
## b. Change name of "preg.art.covered" to "preg.covered"

## 1 Oct 2013: Adding nodematch-type age mixing for bipartite networks
## works (in version d9). BUt nodemix, which would specify the entire
## matrix does not -- "cannot allocate vector" error. So instead
## use nodemix to specify the structural zeros only and see what happens?

# 30 Sep 2013: Work in age-mixing
...
source("../params_uganda_d3.R") #18Nov13
...
#####################################################
  ...
  formation.w.age.mix <- ~edges+b1degree(0:1)+b2degree(0:1)+
                                b1starmix(k=1,
                                          attrname="age.cat",
  ...
  formation.n0 <- update.formula(formation.w.age.mix, n0~.)
  ...
  age.mixing.all.stat.prop <- matrix(c(0.149, 0.013, 0, 0,
                                       0.154, 0.258, 0.018, 0,
                                       0.016, 0.096, 0.115, 0.042,
                                       0.015, 0.027, 0.055, 0.042
                                       ),
                                        nrow=4, ncol=4, byrow=TRUE)

  age.mixing.all.stat <- age.mixing.all.stat.prop*female.deg.tot.matched

    ##        [,1]     [,2]     [,3]   [,4]
    ## [1,] 299.1175  26.0975   0.0000  0.000
    ## [2,] 309.1550 517.9350  36.1350  0.000
    ## [3,]  32.1200 192.7200 230.8625 84.315
    ## [4,]  30.1125  54.2025 110.4125 84.315


  age.mix.spec.diag <- age.mixing.all.stat[to.specify]
  ## men are along rows and women are along columns
  age.b1factor <- apply(age.mixing.all.stat, 2, sum) #sum across columns for
                                                     #men in each age group
  #age.b1factor.spec <- age.b1factor[2:4]
   age.b1factor.spec <- age.b1factor[3:4]

  age.b2factor <- apply(age.mixing.all.stat, 1, sum) #sum across rows for women
                                                     #in each age group
  #age.b2factor.spec <- age.b2factor[2:4]
   age.b2factor.spec <- age.b1factor[3:4]

     # 1 Oct13: correct ordering for age-mixing matrix is
                ## edgesb1deg0b1deg1b2deg0b2deg1b1starmix.1.age.cat.2.1b1starmix.1.age.cat.3.1b1starmix.1.age.cat.4.1b1starmix.1.age.cat.1.2b1starmix.1.age.cat.2.2b1starmix.1.age.cat.3.2b1starmix.1.age.cat.4.2b1starmix.1.age.cat.1.3b1starmix.1.age.cat.2.3b1starmix.1.age.cat.3.3b1starmix.1.age.cat.4.3b1starmix.1.age.cat.1.4b1starmix.1.age.cat.2.4b1starmix.1.age.cat.3.4b1starmix.1.age.cat.4.4.

  target.stats.with.age.diag.b1factor.b2factor   <- c(target.stats,
                                                      age.mix.spec.diag,
                                                      age.b1factor.spec,
                                                      age.b2factor.spec
                                                      )
...
#####################################################
      ...
      ## Age
      prob.for.each.age <- unlist(lapply(age.distribution,
                                         function(x) rep(x, 5)))

      prob.for.each.age <- prob.for.each.age[-c(1:3)]
                           #7Nov13:Remove first 3 entries in vector,
                           #because we are getting rid of ages 15, 16, 17.

      ## then assign the probability for each age class to be selected to
      ## the numeric
      ...
      ## checks for age
      summary(n0%v%"age")
      length(intersect(which(age>=18), which(age<20)))/N # first age class
      length(intersect(which(age>=50), which(age<55)))/N # last age class

      ## 29Sep13: work in age mixing
      ...
      asfr.25to29 <- 0.000836712 ## 12Nov13: See J25-31 in 
      asfr.30to34 <- 0.000691233 ## "fertility rate calculations" in dropbox folder
      asfr.35to39 <- 0.000476164
      asfr.40to44 <- 0.000207945
      asfr.45to49 <- 9.50685E-05

      asfr.15to19 <- asfr.15to19*size.of.timestep ##18Nov13: multiplied above by 
      asfr.20to24 <- asfr.20to24*size.of.timestep ## "size.of.timestep" since above
      asfr.25to29 <- asfr.25to29*size.of.timestep ## are in daily time units.
      ...
      set.vertex.attribute(n0, "num.of.pregnancies", 0)
                                        #4Dec13: set to 0 for everyone initially
      set.vertex.attribute(n0, "num.of.pregnancies", 1,
                           pregnant.yes)
                                        #4Dec13: update to 1 for pregnant at time 0

  ## BIOLOGICAL ATTRIBUTES 

      ## Infection Status 
      init.inf.status <- rbinom(network.size(n0), 1, init.hiv.prev.10)
      ...
      for (i in 1:N){
       ...
       ## n0%v%"vl.art.traj.slope" <- rep(NA, N) #30Jan14
       n0%v%"vl.art.traj.slope" <- vl.art.traj.slope #30Jan14
    ...
    ## 21Feb14: Back-calculate new attributes: CD4 at ART initiation,
    ## VL at ART initiation, CD4 at ART cessation
       n0%v%"cd4.at.art.initiation" <- cd4.count.today -
                                         (time.since.art.initiation*per.day.cd4.recovery*size.of.timestep)
       n0%v%"vl.at.art.initiation"  <- viral.load.today+
                                       abs((undetectable.vl - viral.load.today[on.art[i]])/time.to.full.supp)*
                                       time.since.art.initiation
       n0%v%"cd4.at.art.cessation"  <- NA
       n0%v%"vl.at.art.cessation"  <- NA #21Feb14
...
#####################################################
  ...
  fit <- ergm(formation.n0,
              target.stats=
              target.stats.with.age.diag.b1factor.b2factor,
  ...
  ## 4DEC13: ``control parameter''
  ## COMMENTED OUT, because ergm version has been updated to 3.1-1 from 3.1-0.
  ## this simulate code produces an error in version 3.1-1. Talking to Matt about
  ## reverting to 3.1-0 (cannot do so at the moment because ergm ``resource'' is busy.
...
## Save Object
#####################################################
##  save.image(file="estimation_uganda_d10a_agemix_diag_b1fact_b2fact_minage18_18nov13.RData")
save.image(file="estimation_uganda_d13a.RData") #30Jan14
#####################################################



## 1Oct2013: test age mixing
age.mix.mat <- mixingmatrix(hetdeg.diag.sim, "age.cat")$matrix
round(age.mix.mat/sum(age.mix.mat), 3)
