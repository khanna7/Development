## Update treatment
  ...
  ## 28 Oct 2013: Add new attributes:
  ## (Record (b) in attribute "cd4.of.preg.at.supp.time.art.init" below)
  ...
  ## 28 Oct 2013: Remove values for all function arguments
  ...
  function(nw, verbose,
       ...
       ## eligible at CD4 count ~350 cells/mm3
         ...
         if (scenario == "baseline"){
           ...
           ##   intersect(untreated,
           ##             female.below.cd4.init.realistic)
           ## 12Jul13: these lines above are not needed, and
           ...
           ## vl.art.traj.slope[untreated.bl.initiators] <- 
           ##   abs((undetectable.vl - vl.art.traj.slope[untreated.bl.initiators])/
           ##       time.to.full.supp)

           ## 12Jul13: Above should have viral.load.today[untreated.bl.initiators]
           ## in numerator -- where viral.load.today is the viral load at ART initiation.
           ## browser()
           ...
           set.vertex.attribute(nw, "art.type", 1,
                                untreated.male.below.realistic.cd4) # added 6Jul13
           set.vertex.attribute(nw, "art.type", 1,
                                untreated.female.below.realistic.cd4) # added 6Jul13
           ...
           ###############################################################
           ## 1 Aug 2013: Change lines below to fix error of NA assigned
           ## for "vl.art.traj.slope" in art initiators.
           
           ## 12Jun2013: Add lines below
           ...
           ## for pregnant women
            ...
            preg.at.supp.time.art.init <- 
            which(time.since.curr.pregnancy ==
                  round(baseline.f.ges.visit)) #28Oct13: (b)
           ...
           set.vertex.attribute(nw, "cd4.of.preg.at.supp.time.art.init",
                                cd4.count.today[preg.at.supp.time.art.init], 
                                preg.at.supp.time.art.init) #28Oct13: (b)
         ...
         ## based on time since infection
           ...
           ## stage.of.infection <- rep(NA, length(time.since.infection))
           ## ## 11Jul13: is not needd when "simulation_uganda_d12.R" is run.
           ## ## But needed for unit test.
