...
## 8 Oct 2013: a. Memory problems are still showing up -- one thing to do might be
## to try collapsing the network at every time step. 

## b. To do a properly, network.extract statements have to be taken out
## from elsewhere in the code --
## "update.vital.dynamics_d11a" and "transmission_d9"
...
## STATNET-Related Files and Packages
  ...
  ## SIMULATION-Related Files and Packages
      ...
      source("common.functions_d8.R")
       ...
       source("update.vital.dynamics_d11a_nonetextract.R") # 8Oct13
       ...
       #source("transmission_d9.R") #8Oct13
       source("transmission_d9_nonetextract.R") #8Oct13
...
date <- "8Oct13_idealizedinterv_diagstrzero_age15to55_collapsenet_contn_rmnetextract_del_lgobj_260steps_run1"
  ...
  for (time in 1301:timesteps) {   
    ...
    nw <- compute.cd4.count.sexage.acct(nw, verbose=FALSE,
      ...
      cat("Finished timestep ",time," of ",timesteps,".\n",sep="")
        ...
        if (time >= 1301){
          nw <- network.collapse(nw, at=time) #8Oct13
          cat("Collapsing network ...", "\n")
      }
