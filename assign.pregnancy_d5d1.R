## 1 Nov 2013: Record number of women on different types of ART.
...
assign.pregnancy <-
  ...
  ## Now think about how to populate prob-vector.
   ...
   ######################################################################## 
    ...
    ###########################################################
    ## 31oct13: Record different treatment types among pregnant women
    ## 23 Oct 2013: Record number of new pregnancies
    ###########################################################    
    net <- network.collapse(nw, at=time)
    art.status <- net%v%"art.status"
    art.type <- net%v%"art.type"
    tot.num.pregnancies <- length(which(net%v%"curr.pregnancy.status" == 1))
    female.id.net <- nwmodes(net, 2)

    ##browser()
    
    num.fem.on.art <- length(which(art.status[female.id.net] == 1))
    num.fem.on.reg.art <- length(which(art.type[female.id.net] == 1))
    num.fem.on.sc.art <- length(which(art.type[female.id.net] == 2))
    num.fem.stop.sc.art <- length(which(art.type[female.id.net] == 3))
    
    tot.num.pregnancies <- length(which(net%v%"curr.pregnancy.status" == 1))

    if (verbose==TRUE){

      cat("Number of women on ART are ", num.fem.on.art, "\n")
      cat("Number of women on regular ART are ", num.fem.on.reg.art, "\n")
      cat("Number of women on shortcourse ART are ", num.fem.on.sc.art, "\n")
      cat("Number of women who have stopped scART are", num.fem.stop.sc.art, "\n")
