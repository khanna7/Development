## Update treatment
  ...
  ## 28 April 2014: Option B -- currently everyone getting Option B
  ## is assigned treatment status 4, and therefore stops PMTCT at
  ## 1 year after breastfeeding. Make change so only those with CD4>350
  ## at PMTCT intervention stop treatment 1 year after breastfeeding. 
       ...
       ## eligible at CD4 count ~350 cells/mm3
         ...
         if (scenario == "baseline" || scenario == "pmtct.b+" || scenario == "option.b"){#14Oct13,
          ...
          if (scenario == "option.b"){ ##20Dec13: PMTCT Option B for
            ...
            ind.below.cd4.optB.thres <- which(cd4.count.today < optB.thres)
                                      # new argument
            ind.above.cd4.optB.thres <- which(cd4.count.today >= optB.thres)
            ...
            ##################################################
            preg.init.reg.art <- intersect(preg.initiators,
                                           ind.below.cd4.optB.thres)
            preg.init.sc.art <- intersect(preg.initiators,
                                          ind.above.cd4.optB.thres)
            ...
            ##    time.to.full.supp ## for pregnant women on regular ART
            vl.art.traj.slope[preg.initiators] <-
              abs(undetectable.vl - viral.load.today[preg.initiators])/
                time.to.full.supp ## 28Apr14: ART type 1 and ART type 4 are
                                  ## functionally equivalent, except type 4 is
                                  ## ceased 1 year after initiation, and
                                  ## type 1 continues for life
            ...
            ## browser()
            set.vertex.attribute(nw, "art.type", 1, 
                                preg.init.reg.art) ## 28Apr14:cART for life in preg women
                                                   ## with CD4 count below Opt B threshold
            set.vertex.attribute(nw, "art.type", 4,
                                 preg.init.sc.art) #28Apr14: cART until 1 year post-breastfeeding
                                                   #with CD4 count above Opt B threshold
            set.vertex.attribute(nw, "vl.art.traj.slope",
                                 vl.art.traj.slope[preg.initiators], 
                                 preg.initiators)
