...
## 30 Oct 2013:
## a. Add attributes related to information about the infector
## b. VERY IMPORTANT:
## Change name of attribute from "preg.art.covered" to "preg.covered"

## 28 Oct 2013: Work in these new attributes into estimation

## 23 Oct 2013: Add new attributes:
...
## 4 Oct 2013: Estimate this model with partnership duration restricted
## to ages 15 and 55. 
...
#####################################################
  ...
  theta.diss <- log(duration.15to55 - 1)#4Oct13
...
#####################################################
      ...
      ## Number of pregnancies # 28Oct13
      n0%v%"num.of.pregnancies" <- 0
      set.vertex.attribute(n0, "time.since.curr.pregnancy", 1,
                           pregnant.yes)
  ...
  ## BIOLOGICAL ATTRIBUTES 
      ...
      set.vertex.attribute(n0, "preg.covered", preg.art.covered,
                           v=init.infected.female) # 30Oct13: call attribute
                                                   # "preg.covered" 
    ...
    ## Infectivity today "infectivity.today"
     ...
     ## CD4 count when ART is supposed to be initiated for pregnant women ##28Oct13
        n0%v%"cd4.of.preg.at.supp.time.art.init" <- NA ## 28Oct13

     ## Record characteristics of infector
        ## NA for all initially infected -- so can be set to NA for all of these

        n0%v%"infector.sex" <- NA
        n0%v%"infector.inf.status" <- NA
        n0%v%"infector.art.type" <- NA
        n0%v%"infector.cd4.count" <- NA
        n0%v%"infector.viral.load" <- NA
        n0%v%"infector.preg.status" <- NA
        n0%v%"infector.circum.status" <- NA
        n0%v%"infector.time.since.infection" <- NA
        n0%v%"infector.art.status" <- NA
        n0%v%"infector.art.type" <- NA
...
#####################################################
  ...
  fit <- ergm(formation.n0,
              ...
              #target.stats.with.age.mix.nodiff.b1fac, #30Sep2013
               target.stats.with.age.diag.strzero,
...
## Save Object
#####################################################
  save.image(file="estimation_uganda_d9a_agemix_diagstrzero_restptdurn.RData")
#####################################################
